        ...
        try:
            if account_type == 'advocate':
                conn.execute(
                    ...
                    (first_name, middle_name, last_name, email, account_type, password, approved) 
                    VALUES (?, ?, ?, ?, ?, ?, 0)
                    ''',
                    (first_name, middle_name, last_name, email, account_type, password)
                )
                conn.commit()
                flash('Sign up successful! Await admin approval before logging in.')
...
def admin_dashboard():
    ...
    if request.method == 'POST':
        # Fetch form data including email
        first_name = request.form['first_name']
        last_name = request.form['last_name']
        ...
        conn.execute(
            '''INSERT INTO lawyers 
            (first_name, last_name, district, legal_area_focus, description, past_cases, phone, email, photo) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (first_name, last_name, district, legal_area_focus, description, past_cases, phone, email, filename)
        )
        conn.commit()
        flash('Lawyer added successfully!')


    # Fetch unapproved and approved advocates
    unapproved_advocates = conn.execute('SELECT * FROM advocate WHERE approved = 0 AND declined = 0').fetchall()
    approved_advocates = conn.execute('SELECT * FROM advocate WHERE approved = 1').fetchall()

    # Fetch all lawyers
    lawyers = conn.execute('SELECT * FROM lawyers').fetchall()

    # Debugging the data: print the fetched data
    print(lawyers)  # Check the data in the console
    ...
    return render_template(
        ...
        lawyers=lawyers,
        unapproved_advocates=unapproved_advocates,
        approved_advocates=approved_advocates
    )

@app.route('/lawyer_photo/<int:lawyer_id>')
def lawyer_photo(lawyer_id):
    """Serve lawyer's photo from database."""
    conn = get_db_connection()
    lawyer = conn.execute('SELECT photo FROM lawyers WHERE id = ?', (lawyer_id,)).fetchone()
    conn.close()

    if lawyer and lawyer['photo']:
        return lawyer['photo'], {'Content-Type': 'image/jpeg'}  # Adjust content type based on image format
    else:
        return '', 404


@app.route('/approve_advocate/<int:adv_id>', methods=['POST'])
def approve_advocate(adv_id):
    if 'user_type' not in session or session['user_type'] != 'admin':
        flash('Only admins can approve advocates.')
        return redirect(url_for('login'))

    conn = get_db_connection()
    conn.execute('UPDATE advocate SET approved = 1 WHERE adv_id = ?', (adv_id,))
    conn.commit()
    conn.close()

    flash('Advocate approved successfully!')
    return redirect(url_for('admin_dashboard'))


@app.route('/decline_advocate/<int:adv_id>', methods=['POST'])
def decline_advocate(adv_id):
    if 'user_type' not in session or session['user_type'] != 'admin':
        flash('Only admins can decline advocates.')
        return redirect(url_for('login'))

    conn = get_db_connection()

    # Option 1: Delete the advocate record
    conn.execute('DELETE FROM advocate WHERE adv_id = ?', (adv_id,))
    
    # Option 2: Mark the advocate as declined (you may need to add a "declined" column in your database)
    # conn.execute('UPDATE advocate SET approved = -1 WHERE adv_id = ?', (adv_id,))

    conn.commit()
    conn.close()

    flash('Advocate declined successfully!')
    return redirect(url_for('admin_dashboard'))
...
def add_advocate():
    ...
    if photo:
        photo_binary = photo.read()  # Read the binary data of the image
    else:
        photo_binary = None
    ...
    conn.execute(
        '''INSERT INTO lawyers 
           (first_name, last_name, email, phone, district, legal_area_focus, description, past_cases, photo) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
        (first_name, last_name, email, phone, district, legal_area_focus, description, past_cases, photo_binary)
    )
    conn.commit()
    conn.close()
...
def login():
    if request.method == 'POST':
        ...
        advocate = conn.execute(
            'SELECT * FROM advocate WHERE email = ? AND password = ?', 
            (email, password)
        ).fetchone()
        conn.close()
